...
using ColorPickerMath.ColorWheel;

namespace ColorPicker
{
    public class ColorCircle : ColorPickerBaseOfT<ColorCircleMath>
    {
        public ColorCircle()
        {
        ...
        private static void DrawGrayGradient(ICanvas canvas, RectF dirtyRect)
        {
        ...
        private void DrawSweepGradient(ICanvas canvas, RectF dirtyRect)
        {
        ...
        private void DrawSector(float angle1, float angle2, Color color, ICanvas canvas, RectF dirtyRect)
        {
            var center = dirtyRect.Center;

            var path = new PathF(center);
            path.AddArc(new PointF(0, 0), new PointF(dirtyRect.Width, dirtyRect.Height), angle1 / (2 * MathF.PI) * 360, angle2 / (2 * MathF.PI) * 360, false);
